...
# ------------------- FILE LOCATION FIX -------------------
def get_csv_path():
    """Locate sample_patient_data.csv dynamically."""
    project_root = os.path.dirname(os.path.abspath(__file__))
    csv_path = os.path.join(project_root, "sample_patient_data.csv")
...
def main():
    ...
    if choice == "1":
        ...
        if operation == "encryption":
            aes_encryption(get_csv_path(), write_to_nfc_card_as_ndef)
        ...
        operation = input("Enter operation ('encryption' or 'decryption'): ").strip().lower()
        if operation == "encryption":
            encrypt_with_rsa_aes(get_csv_path(), write_to_nfc_card_as_ndef)
        elif operation == "decryption":
